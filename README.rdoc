...
Копируем jQuery-плагин Gritter (https://github.com/jboesch/Gritter) в проект:
    ...
    rake gritter:install  # Это таск gem-а gritter (https://github.com/RobinBrouwer/gritter) он подключается автоматом
...
Вместо `include_fritter` можно делать сразу `include_gritter_and_jquery` подробнее тут: https://github.com/RobinBrouwer/gritter
...
4. Уйти от gritter, сделать поддержку rails 3.1 assets
